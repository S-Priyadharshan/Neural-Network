...
vector<float> Net::evaluate(const std::vector<float>& input) {
	float* d_input;
	HANDLE_ERROR(cudaMalloc(&d_input, sizeof(float) * input.size()));
	HANDLE_ERROR(cudaMemcpy(d_input, input.data(), sizeof(float) * input.size(), cudaMemcpyHostToDevice));

	feedForward(d_input);

	std::vector<float> result(layers.back().output.size());
	HANDLE_ERROR(cudaMemcpy(result.data(), layers.back().d_output, sizeof(float) * result.size(), cudaMemcpyDeviceToHost));
